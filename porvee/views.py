...
class ProveEdit(BaseUpdate):
 ...
 ########### SECCION DE REGISTRO DE FACTURAS #########################

class FacView(BaseView):
    permission_required = "porvee.view_factura"
    model = Factura
    template_name = "porvee/listfac.html"
...
class FacNew(BaseCreate):
    permission_required = "porvee.add_factura"
    model = Factura
...
class FacEdit(BaseUpdate):
    permission_required = "porvee.update_factura"
    model = Factura
    template_name = "porvee/newfac.html"
    form_class = FacForm
    success_url = reverse_lazy("porvee:listprove")
